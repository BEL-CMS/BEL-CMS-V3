	...
	<div class="card">
		<div class="card-header">
			<div class="flex justify-between items-center">
				<h4 class="card-title"><?=constant('FORUM');?> - <?=constant('CATEGORY');?></h4>
			</div>
		</div>
		<div class="p-6">
				...
				<div class="border rounded-lg overflow-hidden dark:border-gray-700 p-2">
					<table class="DataTableBelCMS min-w-full divide-y divide-gray-200 dark:divide-gray-700 p-2 hover cell-border stripe">
						<thead class="bg-gray-50 dark:bg-gray-700">
							<tr>
								<th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase dark:text-gray-400"><?=constant('TITLE');?></th>
								<th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase dark:text-gray-400"><?=constant('SUBTITLE');?></th>
								<th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase dark:text-gray-400"><?=constant('ACTIVATE');?></th>
						...
						<?php
						foreach ($data as $k => $v):
							$v->activate = $v->activate == 1 ? constant('ACTIVE') : constant('DISABLE');
						?>
							<tr>
								<td><?=$v->title?></td>
								<td><?=$v->subtitle?></td>
								<td><?=$v->activate?></td>
								<td>
									<button class="btn btn-sm bg-primary text-white" onclick="window.location.href='/Forum/EditCat/<?=$v->id?>?management&option=pages'"><i class="mgc_edit_2_fill text-base me-4"></i><?=constant('EDIT');?></button>
									...
									<div id="delete-modal_<?=$v->id?>" class="w-full h-full mt-5 fixed top-0 left-0 z-50 transition-all duration-500 fc-modal hidden"> 
										<div class="fc-modal-open:opacity-100 duration-500 opacity-0 ease-out transition-all sm:max-w-lg sm:w-full m-3 sm:mx-auto flex flex-col bg-white border shadow-sm rounded-md dark:bg-slate-800 dark:border-gray-700">
											<div class="flex justify-between items-center py-2.5 px-4 border-b dark:border-gray-700">
												<h3 class="font-medium text-gray-800 dark:text-white text-lg"><?=$v->title?></h3>
											...
											<div class="px-4 py-8 overflow-y-auto">
												...
												Confirmer la suppression de la categorie : <?=$v->title?>
												</p>
											</div>
											<div class="flex justify-end items-center gap-4 p-4 border-t dark:border-slate-700">
												...
												<a class="btn bg-primary text-white" onclick="window.location.href='/Forum/delCategory/<?=$v->id?>?management&option=pages'"><?=constant('DELETE');?></a>
											</div>
										</div>
									</div>
								</td>

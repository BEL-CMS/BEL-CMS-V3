		...
		<div class="card">
			<div class="card-header">
				<div class="flex justify-between items-center">
					<h4 class="card-title"><?=constant('EDIT');?> - <?=constant('CAT');?></h4>
				</div>
			</div>
			<div class="p-6">
				<div class="overflow-x-auto">
					<div>
						...
						<input name="title" value="<?=$data->title?>" placeholder="Titre de la catégorie" type="text" class="form-input" required="required">
					</div>
					<div>
						<label class="text-gray-800 text-sm font-medium inline-block mt-2 mb-2"><?=constant('SUBTITLE')?></label>
						<input name="subtitle" value="<?=$data->subtitle?>" placeholder="Sous-titre de la catégorie" type="text" class="form-input">
					</div>
					<div>
						<label class="text-gray-800 text-sm font-medium inline-block mt-2 mb-2"><?=constant('ORDER')?></label>
						<input name="orderby" placeholder="1" min="1" type="number" class="form-input" value="<?=$data->orderby?>">
					</div>
					<?php
					$checked = ($data->activate == 1) ? 'checked' : null;
					?>
					<div class="flex items-center mt-2 mb-2">
                        <input type="checkbox" <?=$checked?> value="1" name="activate" id="subcat" class="form-switch text-secondary">
			...
			<div class="p-6">
				...
				<div class="mt-2 mb-2">
					<div class="flex items-center">
						<input id="<?=$v['id']?>" value="<?=$v['id']?>" type="checkbox" name="access_admin[]" class="form-switch square text-success" <?=$checked?>>
		...
		<div class="card">
			...
			<div class="p-6">
				...
				foreach ($groups as $k => $v):
					...
					<div class="mt-2 mb-2">
						<div class="flex items-center">
							<input id="<?=$v['id']?>" value="<?=$v['id']?>" type="checkbox" name="access_groups[]" class="form-switch square text-success" <?=$checked?>>
							<label for="<?=$v['id']?>" class="ms-2"><?=$k?></label>
						</div>
					</div>
					<?php

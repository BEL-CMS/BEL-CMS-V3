...
?>
<form action="pricing/sendEdit?management&option=pages" method="post">
			...
			<div class="p-6">
				<div class="overflow-x-auto">
					<div class="mt-2 mb-2">
						...
						<input name="name" type="text" class="form-input" id="input-name" required="required" value="<?=$data->name;?>">
					</div>
					<div class="mt-2 mb-2">
						<label class="text-gray-800 text-sm font-medium inline-block mt-2 mb-2"><?=constant('ORDER');?></label>
						<input name="sort_asc" type="number" min="1" class="form-input" id="input-name" required="required" value="<?=$data->sort_asc;?>">
					</div>
					<div>
						<div class="relative">
							<input type="number" id="input-with-leading-and-trailing-icon" name="price" class="form-input ps-11 pe-14" placeholder="0.00" value="<?=$data->price;?>">
					...
					<div class="flex">
						...
						<textarea name="description" rows="4" class="form-textarea ltr:rounded-s-none rtl:rounded-e-none mt-2 mb-2"><?=$data->description;?></textarea>
					</div>
					<div>
						...
						<select class="form-select" name="per">
                            <option value="<?=$data->per;?>"><?=constant(strtoupper($data->per));?></option>
			...
			<div class="p-6">
				<div>
					<div class="flex flex-col gap-2">
						<?php
						foreach ($list as $k => $n):
                            $checked = $n->id == $data->listing ? "checked" : '';
						?>
						<div class="form-check">
							<input type="radio" class="form-radio text-primary" name="list" value="<?=$n->id;?>" id="id_<?=$k;?>" <?=$checked;?>>

	...
	<div class="card">
		<div class="card-header">
			<div class="flex justify-between items-center">
				<h4 class="card-title"><?=constant('LIST_OF_GUESTBOOK');?></h4>
			</div>
		</div>
		<div class="p-6">
					...
					<table class="DataTableBelCMS min-w-full divide-y divide-gray-200 dark:divide-gray-700 p-2 hover cell-border stripe">
						...
						foreach ($data as $k => $v):
							...
							<tr class="dark:hover:bg-gray-700">
								...
								<td><?=$v->date_msg?></td>
								<td>
									<button class="belcms_tooltip_left" data="<?=constant('EDIT');?>" onclick="window.location.href='Guestbook/edit/<?=$v->id?>?management&option=pages'"><i class="mgc_edit_line text-lg"></i></button>
									...
									<div id="delete-modal_<?=$v->id?>"
										...
										<div class="fc-modal-open:opacity-100 duration-500 opacity-0 ease-out transition-all sm:max-w-lg sm:w-full m-3 sm:mx-auto flex flex-col bg-white border shadow-sm rounded-md dark:bg-slate-800 dark:border-gray-700">
											<div class="flex justify-between items-center py-2.5 px-4 border-b dark:border-gray-700">
												<h3 class="font-medium text-gray-800 dark:text-white text-lg"><?=$v->author?></h3>
											...
											<div class="px-4 py-8 overflow-y-auto">
												<p class="text-gray-800 dark:text-gray-200">
													<?=constant('DEL_CONFIRM');?> <?=constant('OF2');?> <?=constant('GUESTBOOK');?> : <?=$v->author?>
												</p>
											</div>
											<div class="flex justify-end items-center gap-4 p-4 border-t dark:border-slate-700">
												...
												<a class="btn bg-primary text-white" onclick="window.location.href='Guestbook/del/<?=$v->id?>?management&option=pages'"><?=constant('DELETE');?></a>
											</div>
										</div>
									</div>
								</td>

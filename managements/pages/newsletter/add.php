					...
					<table class="DataTableBelCMS min-w-full divide-y divide-gray-200 dark:divide-gray-700 p-2 hover cell-border stripe">
						<thead class="bg-gray-50 dark:bg-gray-700">
							<tr>
								...
								<th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase dark:text-gray-400"><?=constant('NAME_OF_TPL');?></th>
                        ...
                        <tr>
                            <td><?=$v->id;?></td>
                            <td><?=$v->nametpl->name;?></td>
                            <td><?=Common::TransformDate($v->date, 'LONG', 'SHORT');?></td>
                            <td>
                                <button type="button" class="btn bg-success btn-sm text-white" onclick="window.location.href='newsletter/sendMails/<?=$v->id?>?management&option=pages'">
						            <i class="mgc_mail_send_fill me-3 text-base me-4"></i><?=constant('SEND');?>
                                </button>
								...
								<div id="delete-modal_<?=$v->id?>"
									...
									<div class="fc-modal-open:opacity-100 duration-500 opacity-0 ease-out transition-all sm:max-w-lg sm:w-full m-3 sm:mx-auto flex flex-col bg-white border shadow-sm rounded-md dark:bg-slate-800 dark:border-gray-700">
										<div class="flex justify-between items-center py-2.5 px-4 border-b dark:border-gray-700">
											<h3 class="font-medium text-gray-800 dark:text-white text-lg"><?=$v->id?></h3>
										...
										<div class="px-4 py-8 overflow-y-auto">
											<p class="text-gray-800 dark:text-gray-200">
												<?=constant('DEL_CONFIRM');?> <?=constant('OF2');?> <?=constant('TPL');?> : <?=$v->id?>
											</p>
										</div>
										<div class="flex justify-end items-center gap-4 p-4 border-t dark:border-slate-700">
											...
											<a class="btn bg-primary text-white" onclick="window.location.href='newsletter/deltpl/<?=$v->id?>?management&option=pages'"><?=constant('DELETE');?></a>
										</div>
									</div>
								</div>
                            </td>

	...
	<div class="card">
		<div class="card-header">
			<div class="flex justify-between items-center">
				<h4 class="card-title"><?=constant('LIST_OF_TPL');?></h4>
			</div>
		</div>
		<div class="p-6">
				...
				<div class="border rounded-lg overflow-hidden dark:border-gray-700 p-2">
					<table class="DataTableBelCMS min-w-full divide-y divide-gray-200 dark:divide-gray-700 p-2 hover cell-border stripe">
						...
						<tr>
							...
							<td>
								<button class="btn btn-sm bg-warning text-white" onclick="window.location.href='newsletter/viewtpl/<?=$v->id?>?management&option=pages'">
									<i class="mgc_news_fill text-base me-4"></i><?=constant('VIEW');?>
								</button>
								<button class="btn btn-sm bg-primary text-white" onclick="window.location.href='newsletter/edittpl/<?=$v->id?>?management&option=pages'">
									...
									<div class="fc-modal-open:opacity-100 duration-500 opacity-0 ease-out transition-all sm:max-w-lg sm:w-full m-3 sm:mx-auto flex flex-col bg-white border shadow-sm rounded-md dark:bg-slate-800 dark:border-gray-700">
										...
										<div class="px-4 py-8 overflow-y-auto">
											<p class="text-gray-800 dark:text-gray-200">
												<?=constant('DEL_CONFIRM');?> <?=constant('OF2');?> <?=constant('TPL');?> : <?=$v->name?>
											</p>
										</div>

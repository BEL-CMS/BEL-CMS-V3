...
?>
<form action="Tickets/sendparameter?management&option=pages" method="post">
	<div class="flex flex-col gap-6">
		<div class="grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-6">
			<div class="card">
				<div class="card-header">
					<h4><?=constant('TICKETS');?></h4>
				</div>
				<div class="p-6">
					<div>
						<div class="flex items-center opacity-60">
							<input class="form-switch" <?=$config->active == 1 ? 'checked' : ''?> name="active" type="checkbox"  value="1" role="switch" id="active">
							<label class="ms-1.5" for="active"><?=constant('TICKETS_PAGE_ACTIVE');?></label>
						</div>
					</div>
				</div>
			</div>

	...
	<div class="card">
		<div class="card-header">
			<div class="flex justify-between items-center">
				<h4 class="card-title"><?=constant('LINKS')?> - <?=constant('CATEGORY')?></h4>
			</div>
		</div>
		<div class="p-6">
					...
					<table class="DataTableBelCMS min-w-full divide-y divide-gray-200 dark:divide-gray-700 p-2 hover cell-border stripe">
						<thead class="bg-gray-50 dark:bg-gray-700">
							<tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase dark:text-gray-400"><?=constant('COLOR');?></th>
						...
						foreach ($cat as $v):
							...
							<tr>
								<td><?=$v->id;?></td>
								<td><?=$v->name;?></td>
								<td style="background:<?=$v->color;?>;color:#FFF"><?=$v->color;?></td>
								<td>
									<button class="belcms_tooltip_left" data="<?=constant('EDIT');?>" onclick="window.location.href='Links/editcat/<?=$v->id?>?management&option=pages'"><i class="mgc_edit_line text-lg"></i></button>
										...
										<div class="fc-modal-open:opacity-100 duration-500 opacity-0 ease-out transition-all sm:max-w-lg sm:w-full m-3 sm:mx-auto flex flex-col bg-white border shadow-sm rounded-md dark:bg-slate-800 dark:border-gray-700">
											...
											<div class="px-4 py-8 overflow-y-auto">
												<p class="text-gray-800 dark:text-gray-200">
													<?=constant('DEL_CONFIRM');?> <?=constant('OF2');?> <?=constant('LINKS');?> : <?=$v->name?>
												</p>
											</div>
											<div class="flex justify-end items-center gap-4 p-4 border-t dark:border-slate-700">
												...
												<a class="btn bg-primary text-white" onclick="window.location.href='Links/delcat/<?=$v->id?>?management&option=pages'"><?=constant('DELETE');?></a>
											</div>
										</div>
									</div>
								</td>

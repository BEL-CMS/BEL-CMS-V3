			...
			<tbody>
				...
				foreach ($data as $k => $v):
					...
					<tr>
						...
						<td>
							<button class="btn btn-sm bg-primary text-white" onclick="window.location.href='Shoutbox/edit/<?=$v->id?>?management&option=widgets>'"><i class="mgc_edit_2_fill text-base me-4"></i><?=constant('EDIT');?></button>
								...
								<div class="fc-modal-open:opacity-100 duration-500 opacity-0 ease-out transition-all sm:max-w-lg sm:w-full m-3 sm:mx-auto flex flex-col bg-white border shadow-sm rounded-md dark:bg-slate-800 dark:border-gray-700">
									...
									<div class="px-4 py-8 overflow-y-auto">
										<p class="text-gray-800 dark:text-gray-200">
											<?=constant('DEL_CONFIRM');?> <?=constant('OF2');?> <?=$user->user->username;?> : <?=$v->id?>
										</p>
									</div>

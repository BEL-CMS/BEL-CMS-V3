					...
					<table class="DataTableBelCMS min-w-full divide-y divide-gray-200 dark:divide-gray-700 p-2 hover cell-border stripe">
						<thead class="bg-gray-50 dark:bg-gray-700">
							<tr>
								...
								<th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase dark:text-gray-400"><?=constant('SUBJECT');?></th>
						...
						foreach ($contact as $k => $v):
							...
							$reply = $v->reply === true ? 'Message répondu' : 'Message à répondre';
							$color = $v->reply === true ? 'bg-success/90' : 'bg-dark/80';
                        ?>
							<tr class="dark:hover:bg-gray-700">
                                ...
                                <td><?=$v->mail?></td>
								<td>
									<button class="btn btn-sm <?=$color;?> text-white" onclick="window.location.href='Contact/viewreply/<?=$v->id?>?management&option=parameter'">
										<i class="mgc_edit_2_fill text-base me-4"></i><?=$reply;?>
									</button>
									<button class="btn btn-sm bg-primary text-white" onclick="window.location.href='Contact/view/<?=$v->id?>?management&option=parameter'">
										<i class="mgc_edit_2_fill text-base me-4"></i><?=constant('VIEW');?>
									</button>
										...
										<div class="fc-modal-open:opacity-100 duration-500 opacity-0 ease-out transition-all sm:max-w-lg sm:w-full m-3 sm:mx-auto flex flex-col bg-white border shadow-sm rounded-md dark:bg-slate-800 dark:border-gray-700">
											...
											<div class="px-4 py-8 overflow-y-auto">
												<p class="text-gray-800 dark:text-gray-200">
													<?=constant('DEL_CONFIRM');?> <?=constant('OF2');?> : <?=$v->subject?>
												</p>
											</div>
											<div class="flex justify-end items-center gap-4 p-4 border-t dark:border-slate-700">
												...
												<a class="btn bg-primary text-white" onclick="window.location.href='Contact/del/<?=$v->id?>?management&option=parameter'"><?=constant('DELETE');?></a>
											</div>
										</div>
									</div>
								</td>

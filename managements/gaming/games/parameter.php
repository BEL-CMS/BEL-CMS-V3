...
?>
<form action="games/sendparameter?management&option=gaming" method="post">
	<div class="flex flex-col gap-6">
		<div class="grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-6">
			<div class="card">
				<div class="card-header">
					<h4><?=constant('GAMES_ACTIVE');?></h4>
				</div>
				<div class="p-6">
					<div>
						<div class="flex items-center opacity-60">
							<input class="form-switch" <?=$config->active == 1 ? 'checked' : ''?> name="active" type="checkbox"  value="1" role="switch" id="active">
							<label class="ms-1.5" for="active"><?=constant('GAMES_ACTIVE');?></label>
						</div>
						<div>
							<label class="text-gray-800 text-sm font-medium inline-block mt-2 mb-2" for="<?=constant('NB_GAME')?>">
							<?=constant('NB_GAME')?></label>
							<div class="col-sm-12">
								<input id="input-NB_GAMING" name="MAX_GAMING_PAGE" type="number" class="form-input" type="number" value="<?=$config->config['MAX_GAMING_PAGE']?>" min="1" max="16">
							</div>
						</div>
					</div>
				</div>

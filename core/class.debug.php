...
class Debug
{
	public function __construct($input, $exitAfter = true, $collapse = false)
	{
		$recursive = function($data, $level=0) use (&$recursive, $collapse) {
			...
			if (in_array($type, array("Object", "Array"))) {
				...
				foreach($data as $key => $value) {
					...
					echo $isTerminal ? "[" . $key . "] => " : "<span style='width:150px; float; display: inline-block;color:blue;font-weight:bold;'><i style='width:50px; display: flex-direction: row;'>[" . $key . "]</i><span style='width:25px;float: right; padding-right:5px;'>&nbsp;=>&nbsp;</span></span>";
				...
				echo $isTerminal ? 
						$type . ($type_length !== null ? "(" . $type_length . ")" : "") . "  " : 
						"<span style='color:#666666;width:100px;display: inline-block;'>" . $type . ($type_length !== null ? "(" . $type_length . ")" : "") . "</span>&nbsp;&nbsp;";
...
function debug ($data, $exitAfter = true, $collapse = false) {
	return new Debug($data, $exitAfter, $collapse);
}
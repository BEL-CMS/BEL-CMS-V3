...
final class eMail
{
    ...
    public function fromName ($data = false)
	{
		if (!empty($data)) {
			...
			Notification::error('Erreur, Erreur');
		}
	}

	public function fromMail ($data = false)
	{
		return !empty($data) ? Secure::isMail($data) : $_SERVER['SERVER_ADMIN'];
	}

	public function toMail ($data = null)
	{
		return !empty($data) ? Secure::isMail($data) : '';
	}

	public function SenderName ($data = null)
	{
		return !empty($data) ? Common::VarSecure($data, 'null') : '';
	}
	...
	public function onYourMind ($data = null)
	{
		if ($data != null) {
			return Common::VarSecure($data, 'html');
		} else {
			return '';
		}
	}
	...
	public function send ()
	{
		...
		$headers  = "From: $email" . PHP_EOL_MAIL;
